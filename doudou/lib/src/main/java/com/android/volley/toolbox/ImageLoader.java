...
import com.android.volley.Response.ProgressListenr;
...
public class ImageLoader {
	 ...
	 */
	public ImageContainer get(String requestUrl, ImageListener imageListener,
			int maxWidth, int maxHeight, ProgressListenr progressListenr) {
		...
		Request<Bitmap> newRequest = makeImageRequest(requestUrl, maxWidth,
				maxHeight, cacheKey, progressListenr);
	...
	protected Request<Bitmap> makeImageRequest(String requestUrl, int maxWidth,
			...
			ProgressListenr progressListenr) {
		return new ImageRequest(requestUrl, new Listener<Bitmap>() {
			@Override
			public void onResponse(Bitmap response) {
				onGetImageSuccess(cacheKey, response);
			}
		}, maxWidth, maxHeight, Config.RGB_565, progressListenr,
				new ErrorListener() {
					@Override
					public void onErrorResponse(VolleyError error) {
	...
	private class BatchedImageRequest {
		...
		private boolean mInProgressing;
